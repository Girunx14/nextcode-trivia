...
class GameConfig:
    ...
    wrist_y_threshold_ratio: float = 0.55  # wrist must be above this line to count as "raised"
...
class RaiseHandGameMulti:
    def __init__(self, cam: int, width: int, height: int, players: int, mirror: bool, cfg: GameConfig):
        ...
        self.hands = self.mp_hands.Hands(
            ...
            max_num_hands=2,  # fiable 1-2 manos simultáneas
            model_complexity=1,
        ...
        self.target = "LEFT"
        self.deadline = 0.0
        self.wait_next = True  # arrancar en pausa inicial para que presiones "Siguiente"

        # feedback pinned until next
    ...
    def _draw_next_button(self, frame, enabled: bool, hover: bool):
        ...
        # hint
        if enabled:
            cv2.putText(frame, "Baja la mano y toca el boton con la muñeca",
                        (x1 - 140 if x1 > 140 else 20, y1 - 10),
    ...
    def _process_choice(self, player_i: int, choice: str):
        ...
        if choice == self.target:
            st.score += 1
            self.round_result_text = f"J{st.idx}: ✅ CORRECTO (+1). Baja las manos y presiona SIGUIENTE."
        else:
            self.round_result_text = f"J{st.idx}: ❌ INCORRECTO. Baja las manos y presiona SIGUIENTE."
    ...
    def run(self):
        ...
        while True:
            ...
            # --- Detect hands (both in game and pause, but different behavior) ---
            rgb = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
            res = self.hands.process(rgb)

            # Reset detected labels each frame
            for st in self.states:
                st.detected_hand = "—"

            # Collect wrist points per hand, per lane
            # We will use:
            # - "raised" logic only when in game
            # - "next button press" logic only when paused, and only when wrist is BELOW y_thr
            raised_left = [False] * self.players
            raised_right = [False] * self.players

            any_wrist_in_next = False
            any_hand_raised = False

            if res.multi_hand_landmarks and res.multi_handedness:
                for lm, hd in zip(res.multi_hand_landmarks, res.multi_handedness):
                    label = hd.classification[0].label  # "Left" / "Right" raw
                    conf = hd.classification[0].score

                    wx = int(lm.landmark[0].x * w)  # wrist
                    ...
                    # require wrist BELOW the raise line (hand down), and inside NEXT rect
                    if self.wait_next and self.next_rect is not None:
                        if (conf >= self.cfg.min_raise_confidence) and (wy >= y_thr) and self._rect_contains(self.next_rect, wx, wy):
                            any_wrist_in_next = True
            ...
            if not self.wait_next:
                ...
                # draw NEXT button disabled (still show it lightly)
                self._draw_next_button(frame, enabled=False, hover=False)

            else:
                # Pause: enable NEXT only if hands are NOT raised
                next_enabled = not any_hand_raised

                hover = False
                if next_enabled and any_wrist_in_next:
                    hover = True
                    if self.next_hover_start is None:
                        ...
                        # IMPORTANT: round index increments here
                        # (handled in _start_new_round)
                else:
                    self.next_hover_start = None

                self._draw_next_button(frame, enabled=next_enabled, hover=hover)

            cv2.imshow("Raise Hand Game (Multi + Next Button)", frame)
...
def main():
    ...
    ap.add_argument("--next-hold", type=float, default=0.25, help="segundos sosteniendo muñeca en SIGUIENTE")
    args = ap.parse_args()
